...
template <typename Scalar, typename Index>
std::array<lagrange::SurfaceMesh<Scalar, Index>, 2>
compute_swept_volume_from_envelope(
    const lagrange::SurfaceMesh<Scalar, Index>& envelope) {
    using Point = Eigen::Matrix<Scalar, 3, 1>;
    ...
    std::vector<Index> valid_parent_facets;
    valid_parent_facets.reserve(num_valid_facets);
    Index count = 0;
    for (size_t fid = 0; fid < num_facets; fid++) {
        if (is_valid[fid] == 0) continue;
        auto f = sweep_surface.ref_facet_vertices(count++);
        valid_parent_facets.push_back(parent_facets(fid));
        if (is_valid[fid] == 1) {
    ...
    sweep_surface.template create_attribute<Scalar>(
        "time", lagrange::AttributeElement::Corner,
        lagrange::AttributeUsage::Scalar, 1);
    auto time_values = attribute_vector_ref<Scalar>(sweep_surface, "time");
    count = 0;
    // TODO: parallalize this loop
    for (size_t fid = 0; fid < num_facets; fid++) {
        if (is_valid[fid] == 0) continue;
    ...
    lagrange::remove_isolated_vertices(sweep_surface);
    return {sweep_arrangement, sweep_surface};
}
